...
class User(Base):
    ...
    # Store the hashed password explicitly in `password_hash` column.
    # Keep a write-only `password` property for ergonomic assignment.
    ...
    # Provide a write-only `password` attribute that will hash raw passwords
    # on assignment. If a hashed string (starting with '$') is provided, store
    # it as-is so tests that pass pre-hashed values continue to work.
    @property
    def password(self) -> None:  # read access is intentionally disabled
    ...
    def password(self, raw: str) -> None:
        if raw is None:
            ...
            # assume already-hashed value
            self.password_hash = raw
        else:
            self.password_hash = self.hash_password(raw)
    ...
    def register(cls, db, user_data: Dict[str, Any]) -> "User":
        ...
        try:
            ...
            new_user = cls(
                ...
                # assign raw password so the model's setter hashes it
