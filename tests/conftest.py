...
# If the primary (Postgres) engine is not available, fall back to an in-memory SQLite
# engine and patch app.database so other modules that import its engine/session
# will use the fallback during tests.
test_engine = None
TestingSessionLocal = None
try:
    test_engine = get_engine(database_url=settings.DATABASE_URL)
    # Quick connect-check to fail fast if Postgres isn't running
    conn = test_engine.connect()
    conn.close()
    TestingSessionLocal = get_sessionmaker(engine=test_engine)
except Exception:
    logger.warning("Primary test engine not connectable; switching to in-memory SQLite for tests.")
    sqlite_url = os.environ.get("TEST_SQLITE_URL", "sqlite:///:memory:")
    test_engine = create_engine(sqlite_url)
    TestingSessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=test_engine)

# Patch app.database module so init_db / other imports use our test engine/session
...
def setup_test_database(request):
    ...
    # Drop all tables to ensure a clean slate for the models' metadata
    ...
    if preserve_db:
        ...
        # Drop the tables created for tests
        ModelBase.metadata.drop_all(bind=test_engine)
